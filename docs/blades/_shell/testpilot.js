...
(function () {
  ...
  function injectCss() {
    ...
    st.textContent = [
      ...
      // 2026-08-13 — moved from top:78px down to ride ABOVE the bottom-right deck cluster
      // (zoom + fullscreen + ↵ GAME), so the floating controls are one group.
      // ★ ANCHORED FROM THE BOTTOM, INDEPENDENTLY — it is NOT in the cluster's layout flow.
      // That is the whole point: this badge comes and goes as the test track is armed, and if
      // the ↵ GAME button were stacked relative to it, disarming would shift the button out
      // from under the pilot's cursor. Bottom-anchoring means the space above simply empties.
      // 62px = cluster bottom 10 + its 46px height + a 6px gap.
      "#obTpTrack{position:fixed;bottom:62px;right:12px;z-index:99998;font-family:var(--font-head,'Orbitron',sans-serif);font-size:9.5px;letter-spacing:2px;color:var(--good,#57e0a0);border:1px solid var(--good,#57e0a0);border-radius:4px;padding:3px 8px;background:rgba(6,10,7,.72);pointer-events:none;transform-origin:right center;animation:obTpBreathe 4.8s ease-in-out infinite}",
